...
COPY --from=build /usr/local/lib/php/extensions /usr/local/lib/php/extensions COPY --from=build /usr/local/bin/composer /usr/local/bin/composer COPY --from=build /var/www/html /var/www/html COPY --from=build /usr/local/etc/php /usr/local/etc/php
...
WORKDIR /var/www/html RUN chown -R www-data:www-data /var/www/html
